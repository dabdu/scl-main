...
const Navbar = () => {
  ...
  return (
    <div className="w-screen h-[80px] z-10 bg-white drop-shadow-lg px-8 md:px-20">
        ...
        <div className="hidden md:flex pr-4">
          <ul className="hidden md:flex">
            {menu_items.map((menu, index) => (
              <li key={index}>
                <Link to={menu.link} smooth={true} duration={500} className="text-black">
                  {menu.name}
                </Link>
              </li>
            ))}
      ...
      <ul
        className={!nav ? "hidden" : "absolute bg-white w-full px-8"}
        style={{ zIndex: 1000 }}
      >
        {menu_items.map((menu, index) => (
